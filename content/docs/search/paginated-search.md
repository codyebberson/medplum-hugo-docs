...
For self-hosted customers, this threshold is server-level configuration called `accurateCountThreshold` ([learn more](/docs/self-hosting/server-config)).

When `_total=accurate` is used, the server will attempt to provide an exact count of the total number of matching resources. For performance reasons, the server may be configured to reject this request if the number of matching resources exceeds a predefined threshold.
